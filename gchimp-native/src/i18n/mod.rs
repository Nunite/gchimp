...
lazy_static! {
    static ref TRANSLATIONS: HashMap<Language, HashMap<TextKey, &'static str>> = {
        ...
        en.insert(TextKey::ConvertSMDHint, "Creates new SMD file that will use those new texture files accordingly");
        en.insert(TextKey::CompileMDLHint, "Creates QC file and compiles the model with included studiomdl.exe");
        en.insert(TextKey::SplitModelTitle, "Split Model");
        en.insert(TextKey::SplitModelHint, "Splits a complete .qc linked with ONE smd to produce more smd with less triangles");
        en.insert(TextKey::LoopWaveTitle, "Loop Wave");
        en.insert(TextKey::LoopWaveHint, "Make a wav file loop");
        en.insert(TextKey::LoopCheckboxHint, "Loop Checkbox Hint");
        en.insert(TextKey::ResMakeTitle, "ResMake");
        en.insert(TextKey::ResMakeHint, "Basically a resgen clone");
        ...
        zh.insert(TextKey::ConvertSMDHint, "创建新的SMD文件，使用相应的新纹理文件");
        zh.insert(TextKey::CompileMDLHint, "创建QC文件并使用包含的studiomdl.exe编译模型");
        zh.insert(TextKey::SplitModelTitle, "分割Smd");
        zh.insert(TextKey::SplitModelHint, "将一个完整的.qc链接到一个smd，这个smd将会被分割成多个smd，每个smd的三角形数量将会减少（但不会减少顶点数量，规避smd的顶点限制）");
        zh.insert(TextKey::LoopWaveTitle, "音频循环");
        zh.insert(TextKey::LoopWaveHint, "使一个wav文件循环");
        zh.insert(TextKey::LoopCheckboxHint, "循环复选框提示");
        zh.insert(TextKey::ResMakeTitle, "ResMake");
        zh.insert(TextKey::ResMakeHint, "一个resgen的克隆");
